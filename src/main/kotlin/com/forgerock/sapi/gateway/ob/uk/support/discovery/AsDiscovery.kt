...
private fun getAsConfiguration(): AsDiscovery {
    val asDiscoveryUrl = "$IG_SERVER/am/oauth2/realms/root/realms/alpha/.well-known/openid-configuration"
    val (_, response, result) = Fuel.get(asDiscoveryUrl)
        .responseObject<AsDiscovery>()
    if (!response.isSuccessful) {
        throw AssertionError(
            "Failed to load As Discovery from $asDiscoveryUrl. Error was ${response.statusCode}",
            result.component2()
        )
    }

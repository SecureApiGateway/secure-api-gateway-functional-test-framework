...
class AccountRS {
    ...
    val ACCOUNT_SCOPES: String = "openid accounts"

    inner class ConsentCreator {
        ...
        inline fun <reified T : Any> consent(
            consentUrl: String, consentRequest: Any, apiClient: ApiClient,
            apiUnderTest: ApiUnderTest
        ): T {
            val accessToken = apiUnderTest.oauth2Server.getAccessToken(apiClient, ACCOUNT_SCOPES)
            val (_, consentResponse, r) = apiClient.fuelManager.post(consentUrl)
                .jsonBody(consentRequest)
                .header("Authorization", "Bearer ${accessToken.access_token}")
                .responseObject<T>()
            if (!consentResponse.isSuccessful) throw AssertionError(
                "Could not create consent: ${String(consentResponse.data)} for apiClient '${apiClient.name}', with " +
                        "api '${apiUnderTest.name}'",
                r.component2()
            )
            return r.get()
        }
...
//    }
    ...
    inline fun <reified T : Any> getConsent(
        consentUrl: String,
        apiClient: ApiClient,
        apiUnderTest: ApiUnderTest
    ): T {
        val accessToken = apiUnderTest.oauth2Server.getAccessToken(apiClient, ACCOUNT_SCOPES).access_token
        val (_, consentResponse, r) = apiClient.fuelManager.get(consentUrl)
            .header("Authorization", "Bearer $accessToken")
            .responseObject<T>()
        if (!consentResponse.isSuccessful) throw AssertionError(
    ...
    fun deleteConsent(consentUrl: String, apiClient: ApiClient, apiUnderTest: ApiUnderTest) {
        val accessToken = apiUnderTest.oauth2Server.getAccessToken(apiClient, ACCOUNT_SCOPES).access_token
        val (_, consentResponse, r) = apiClient.fuelManager.delete(consentUrl)
            .header("Authorization", "Bearer $accessToken")
            .response()
    ...
    inline fun <reified T : Any> getAccountsData(
        ...
        val (_, accountResult, r) = apiClient.fuelManager.get(accountDataUrl)
            .header("Authorization", "Bearer ${accessToken.access_token}")
    ...
    inline fun <reified T : Any> getAccountData(
        ...
        val xObURL = "$IG_SERVER/${accountDataUrl.substring(accountDataUrl.indexOf("rs/") + 3)}"
        val (_, accountResult, r) = apiClient.fuelManager.get(
    ...
    fun getAccountStatementFileData(
        ...
        return apiClient.fuelManager.get(accountDataUrl)
            .header("Authorization", "Bearer ${accessToken.access_token}")
    ...
    fun getFirstAccountIdAndPsuId(
        accountDataUrl: String,
        accessToken: AccessToken,
        apiClient: ApiClient
    ): Pair<String, String> {
        val accounts = getAccountsData<OBReadAccount6>(accountDataUrl, accessToken, apiClient)

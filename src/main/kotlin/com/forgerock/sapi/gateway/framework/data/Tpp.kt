...
import com.forgerock.sapi.gateway.framework.apiclient.RegistrationRequest
...
data class Tpp(
    val softwareStatement: SoftwareStatement, val privateTransportCert: String,
    val publicTransportCert: String, val signingKid: String, val signingKeyPath: String
) {
    ...
    private fun getTransportCertSubjectDN(): String {
        val certStream: InputStream = File(publicTransportCert).inputStream()
    ...
    fun generateSsa(): String {
        val jws = getJWS()
        if (jws != null) {
            println("serializedJWT is ${jws.serialize()}")
    ...
    private fun getJWS(): SignedJWT? {
        if (File(OB_TPP_EIDAS_SIGNING_KEY_PATH).exists()) {
            val detachedPayload =
                Payload(GsonUtils.gson.toJson(com.forgerock.sapi.gateway.ob.uk.framework.accesstoken.model.ClaimsTest()))
            ...
            val jwsHeader: JWSHeader = JWSHeader.Builder(JWSAlgorithm.RS256)
    ...
    private fun acquireAccessToken(jws: SignedJWT): com.forgerock.sapi.gateway.ob.uk.framework.accesstoken.model.AccessTokenResponse {
        val accessTokenRequest =
            com.forgerock.sapi.gateway.ob.uk.framework.accesstoken.model.AccessTokenRequest(client_assertion = jws.serialize())
        ...
        val (_, certResult, r) = Fuel.post(
            com.forgerock.sapi.gateway.ob.uk.framework.configuration.TOKEN_URL_SANDBOX,
            parameters
        )

...
class CreateTppCallback : BeforeAllCallback, BeforeEachCallback, ParameterResolver {
    ...
    override fun beforeEach(context: ExtensionContext?) {
        ...
        tpp?.let {
            initFuel(
                FileUtils().getInputStream(it.tpp.privateTransportCert),
                FileUtils().getInputStream(it.tpp.publicTransportCert)
            )
        }
    }
    ...
    override fun beforeAll(context: ExtensionContext) {
        ...
        initFuel(
            FileUtils().getInputStream(tpp.tpp.privateTransportCert),
            FileUtils().getInputStream(tpp.tpp.publicTransportCert)
        )
    }
    ...
    class TppResource(val tpp: Tpp) : ExtensionContext.Store.CloseableResource {
        ...
        override fun close() {
            ...
            initFuel(
                FileUtils().getInputStream(tpp.privateTransportCert),
                FileUtils().getInputStream(tpp.publicTransportCert)
            )
            tpp.unregister()
        }
    }
